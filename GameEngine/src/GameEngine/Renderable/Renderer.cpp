...
/// </param>
void GameEngine::Renderer::submit(bgfx::ViewId view, bgfx::ProgramHandle prog, uint64_t STATE)
{
...
void GameEngine::Renderer::parseObj(char* filename)
{
    std::list<GameEngine::Vector3> vertices;
    std::list<float> colors;
    ...
    for (int i = 0; i < this->v_len; i++) {
        ...
        vertices.pop_front();
        uint32_t color = RGB(rand() % 255, rand() % 255, rand() % 255);
        this->vertices[i] = PosColorVertex{ v.x, v.y, v.z, color };
    }
...
void GameEngine::Renderer::print()
{
